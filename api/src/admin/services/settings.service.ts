...
export class SettingsService {
  ...
  async addSettings(data: any) {
    ...
    if (add_settings?.length) {
      new_settings = await this.prismaService.setting.createMany({
        data: add_settings,
      });
    }
  ...
  // async sortMenuItems(data) {
  //   const response = { update: 0, failed: 0 };
  //   for (const value of data) {
  //     const update = await this.prismaService.siteMenuItem.update({
  //       where: {
  //         id: value?.id,
  //       },
  //       data: {
